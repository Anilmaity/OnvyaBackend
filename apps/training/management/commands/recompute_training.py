...
from apps.training.services import CompletionService


class Command(BaseCommand):
    help = "Recompute Completion.status across all agencies."

    def handle(self, *args, **opts):
        total = 0
        for agency in Agency.objects.all():
            with agency_context(agency):
                changed = CompletionService.recompute_all(agency)
                total += changed
